...
class SpotifyManager:
    ...
    async def initialize_clients(self):
        logging.info(f"Spotify: Mengaktifkan Stealth Mode via Proxy...")
        os.makedirs(self.conf_path, exist_ok=True)

        # Bersihkan cache lama agar tidak ada sesi 403 yang nyangkut
        ...
        # Buat Device ID acak agar setiap deploy terlihat seperti perangkat baru
        ...
        settings_data = {
            ...
            "device_name": "iPhone 15 Pro", # Menyamar jadi HP
            "device_id": device_id,
            "proxy": self.proxy,
            "bitrate": 320,
            "is_premium": True
        ...
        try:
            # Injeksi proxy ke environment untuk library pendukung
    ...
    async def complete_login(self, url):
        try:
            ...
            async with httpx.AsyncClient(proxy=self.proxy, follow_redirects=True) as client:
                resp = await client.post("https://accounts.spotify.com/api/token4", data={
                ...
                me = await client.get("https://accounts.spotify.com/api/token5", 
                ...
                logging.info(f"Spotify: Login Berhasil! Wilayah Akun: {country}")

...
async def start_track(item_id: str, user: dict, track_meta: dict | None, upload=True, filepath=None, disable_link=False):
    ...
    # Sedikit delay acak (Anti-Ban ringan) sebelum Aria2 mengeksekusi request
    try:
    ...
    # Penyamaran Aria2 agar tidak terkena 403 Forbidden dari server Beatsource
    headers_dict = {"User-Agent": APP_USER_AGENT, "Accept": "*/*", "Referer": "https://www.beatsource.com/"}
    ...
    if err or not os.path.exists(filepath) or os.path.getsize(filepath) < 10000:
        LOGGER.warning(f"Beatsource: Aria2 gagal/403 untuk {track_meta['title']}. Menyegarkan URL...")
        new_url, qual = await refresh_track_url(item_id, track_meta, user_id)
        if new_url:
            track_meta['download_url'] = new_url
